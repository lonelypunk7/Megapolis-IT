...
class MCPProxy:
    ...
    async def route(self, resource, args, method="GET", resource_id=None):
        mapping = {
        ...
        # Determine if this is a single-resource or collection request
        # /resource/id → single, /resource → collection
        is_single = bool(resource_id)
        if is_single:
            # For /suites/346, resource="suites", we need to find singular tool
            # Check if resource maps to a singular key
            singular_map = {
                "suites": "suite", "cases": "case", "tests": "test",
                "projects": "project", "testplans": "testplan",
                "results": "result", "users": "user", "groups": "group",
                "labels": "label", "statuses": "status",
                "attachments": "attachment", "comments": "comment",
                "notifications": "notification",
            }
            base = singular_map.get(resource, resource)
            tool = mapping.get(base, base)
        else:
            tool = mapping.get(resource, resource)
        if method == "GET":
            pass  # tool already set above
        elif method == "POST":
...
async def handle(request):
    try:
        ...
        if request.method == "GET":
            if resource_id:
                # Use singular key for single-resource lookups
                singular_map = {
                    "suites": "suite", "cases": "case", "tests": "test",
                    "projects": "project", "testplans": "testplan",
                    "results": "result", "users": "user", "groups": "group",
                ...
                singular_key = singular_map.get(resource, resource)
                args[singular_key] = resource_id
            result = await proxy.route(resource, args, "GET", resource_id)
        elif request.method == "POST":
            body = await request.json()
            if resource_id:
                body[resource] = resource_id
            result = await proxy.route(resource, body, "POST", resource_id)
        elif request.method == "PUT":
            body = await request.json()
            if resource_id:
                body[resource] = resource_id
            result = await proxy.route(resource, body, "PUT", resource_id)
        elif request.method == "DELETE":
            if resource_id:
                args[resource] = resource_id
            result = await proxy.route(resource, args, "DELETE", resource_id)
        else:
