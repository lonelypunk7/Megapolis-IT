...
async def handle_tool_call(name: str, args: dict, client: TestYClient) -> dict:
    """Route tool calls to methods."""
    try:
        method = getattr(client, name, None)
        if not method:
            # Fallback for methods with underscores that map to API endpoints
            ...
            if name == "copy_case":
                return await client.post(f"/api/v2/cases/{args['case_id']}/copy/", body=args.get("body", args.get("body", {})))
            ...
            if name == "update_user":
                return await client.put(f"/api/v2/users/{args['user_id']}/", body=args.get("body", {}))
            if name == "delete_user":
                return await client.delete(f"/api/v2/users/{args['user_id']}/")
            if name == "get_group":
                return await client.get(f"/api/v2/groups/{args['group_id']}/")
            if name == "update_group":
                return await client.put(f"/api/v2/groups/{args['group_id']}/", body=args.get("body", {}))
            if name == "delete_group":
                return await client.delete(f"/api/v2/groups/{args['group_id']}/")
            if name == "get_label":
                return await client.get(f"/api/v2/labels/{args['label_id']}/")
            if name == "update_label":
                return await client.put(f"/api/v2/labels/{args['label_id']}/", body=args.get("body", {}))
            if name == "delete_label":
                return await client.delete(f"/api/v2/labels/{args['label_id']}/")
            if name == "get_status":
                return await client.get(f"/api/v2/statuses/{args['status_id']}/")
            if name == "update_status":
                return await client.put(f"/api/v2/statuses/{args['status_id']}/", body=args.get("body", {}))
            if name == "delete_status":
                return await client.delete(f"/api/v2/statuses/{args['status_id']}/")
            if name == "get_attachment":
                return await client.get(f"/api/v2/attachments/{args['attachment_id']}/")
            if name == "delete_attachment":
                return await client.delete(f"/api/v2/attachments/{args['attachment_id']}/")
            ...
            if name == "get_case":
                return await client.get(f"/api/v2/cases/{args['case_id']}/")
            if name == "get_test":
                return await client.get(f"/api/v2/tests/{args['test_id']}/")
            if name == "get_suite":
                return await client.get(f"/api/v2/suites/{args['suite_id']}/")
            if name == "delete_test":
                return await client.delete(f"/api/v2/tests/{args['test_id']}/")
            if name == "delete_testplan":
                return await client.delete(f"/api/v2/testplans/{args['plan_id']}/")
            if name == "delete_result":
                return await client.delete(f"/api/v2/results/{args['result_id']}/")
            if name == "delete_comment":
                return await client.delete(f"/api/v2/comments/{args['comment_id']}/")
            if name == "delete_project":
                return await client.delete(f"/api/v2/projects/{args['project_id']}/")
            if name == "delete_attachment":
                return await client.delete(f"/api/v2/attachments/{args['attachment_id']}/")
            if name == "delete_case":
                return await client.delete(f"/api/v2/cases/{args['case_id']}/")
            if name == "delete_suite":
                return await client.delete(f"/api/v2/suites/{args['suite_id']}/")
            if name == "delete_status":
                return await client.delete(f"/api/v2/statuses/{args['status_id']}/")
            if name == "delete_label":
                return await client.delete(f"/api/v2/labels/{args['label_id']}/")
        ...
        if name in ("login", "logout", "get_me", "get_system_stats", "get_plugins", "get_custom_attr_content_types", "mark_notification_read"):
            return await method()
        elif name in ("get_projects", "get_users", "get_groups", "get_labels", "get_statuses", "get_cases", "get_tests", "get_suites", "get_testplans", "get_results", "get_comments", "get_notifications", "get_attachments"):
            page = args.get("page", 1)
            page_size = args.get("page_size", 20)
            extra = {k: v for k, v in args.items() if k not in ("page", "page_size")}
            if extra:
                params = {"page": page, "page_size": page_size, **extra}
            else:
                params = {"page": page, "page_size": page_size}
            return await method(params=params)
        elif name in ("get_project", "get_case", "get_test", "get_suite", "get_testplan", "get_result", "get_user", "get_group", "get_attachment"):
            id_key = name.replace("get_", "").rstrip("s")
            id_val = args.get(id_key) or args.get(f"{id_key}_id")
            if not id_val:
                return {"error": f"Missing {id_key} ID"}
            return await method(f"/api/v2/{id_key}/{id_val}/")
        elif name in ("create_project", "create_case", "create_test", "create_suite", "create_testplan", "create_result", "create_user", "create_group", "create_label", "create_status", "create_attachment", "create_comment"):
            return await method(body=args.get("body", {}))
        elif name in ("update_project", "update_case", "update_test", "update_suite", "update_testplan", "update_result", "update_user", "update_group", "update_label", "update_status", "update_comment"):
            id_key = name.replace("update_", "").rstrip("s")
            id_val = args.get(id_key) or args.get(f"{id_key}_id")
            body = args.get("body", {})
            return await method(f"/api/v2/{id_key}/{id_val}/", body=body)
        elif name in ("delete_project", "delete_case", "delete_test", "delete_suite", "delete_testplan", "delete_result", "delete_user", "delete_group", "delete_label", "delete_status", "delete_attachment", "delete_comment"):
            id_key = name.replace("delete_", "").rstrip("s")
            id_val = args.get(id_key) or args.get(f"{id_key}_id")
            return await method(f"/api/v2/{id_key}/{id_val}/")
        else:
