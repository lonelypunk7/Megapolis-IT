...
async def handle_tool_call(name: str, args: dict, client: TestYClient) -> dict:
    ...
    try:
        ...
        if name in get_by_id:
            ...
            # Use plural path segments for API
            path_segment = id_key + "s" if id_key not in ("project", "case", "test", "suite", "testplan", "result", "user", "group", "attachment") else id_key
            if id_key in ("project", "case", "test", "suite", "testplan", "result", "user", "group", "attachment"):
                path_segment = {
                    "project": "projects", "case": "cases", "test": "tests",
                    "suite": "suites", "testplan": "testplans",
                    "result": "results", "user": "users", "group": "groups",
                    "attachment": "attachments",
                }[id_key]
            return await client.get(f"/api/v2/{path_segment}/{id_val}/")
